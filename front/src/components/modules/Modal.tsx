...
export function Modal() {
  ...
  const renderModalContent = (
    content: JSX.Element | (() => JSX.Element) | null | undefined
  ) => {
  ...
  return (
    <>
      {show && (
        <div id={`modal`} className="fixed inset-0 z-50 overflow-y-auto">
...
export function ModalHeaderLogo({
  color,
  children,
}: {
  color: string;
  ...
  return (
    <div className="flex justify-center mt-3">
      <div className={`rounded-full p-6 ${colorClass}`}>{children}</div>
    </div>
  );
}
