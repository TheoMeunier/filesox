...
export function ModalShareFormMedia() {
  ...
  return (
    <>
      <ModalBody>
        <FormFields
          id="form-share-storage"
          onSubmit={form.handleSubmit(onSubmit)}
        >
          <FormField>
            <FormLabel htmlFor="share_duration">
            ...
            <Row cols={2}>
              ...
              <FormField>
                ...
                >
                  <option value="hours">
                    {t('input.options.shares.hours')}
                  </option>
                  <option value="days">{t('input.options.shares.days')}</option>
                  <option value="weeks">
                    {t('input.options.shares.weeks')}
                  </option>
                  <option value="months">
                    {t('input.options.shares.months')}
                  </option>
                  <option value="years">
                    {t('input.options.shares.years')}
                  </option>
                </select>
                <FormError>
                  {form.formState.errors.type_duration?.message}
                </FormError>
              </FormField>
            </Row>
          </FormField>
          <FormField>
            <FormLabel htmlFor="password ">
              {t('input.label.optional_password')}
            </FormLabel>
            <FormField>
              <input
                type="password"
      ...
      <ModalFooter>
        <Button color="primary" type="submit" form="form-share-storage">
          {t('button.share')}
        </Button>
      </ModalFooter>
    </>
