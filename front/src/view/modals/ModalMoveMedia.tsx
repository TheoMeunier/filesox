...
export function ModalMoveMedia() {
  ...
  return (
    <>
      <ModalHeader>
        <h2 className="flex items-center gap-2 text-2xl">
          <span className="text-indigo-500">
            <MoveUpRight height={28} width={28} />
          </span>
      ...
      <ModalBody>
        <FormFields
          id="form-move-storage"
          onSubmit={form.handleSubmit(onSubmit)}
        >
          <FormDescription>
            {t('input.description.move_storage')}: <TypoCode>./</TypoCode>.
          </FormDescription>
          <FormLabel htmlFor="name">{t('input.label.path')}</FormLabel>
          <FormField>
            <input
              {...form.register('path')}
              type="text"
      ...
      <ModalFooter>
        <Button color="primary" type="submit" form="form-move-storage">
          {t('button.move')}
        </Button>
      </ModalFooter>
    </>
